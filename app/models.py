...
class TutorRoom(db.Model):
    ...
    course = db.relationship("Course", backref = "course_room")
    
    room_id = db.Column(db.String, db.ForeignKey('room.room_id'))
...
class StudentPreference(db.Model):
    ...
    course = db.relationship("Course", foreign_keys=[stucourse_id_2], backref = "stu_course2")
...
class TutorPreference(db.Model):
    ...
    course = db.relationship("Course", foreign_keys=[preference_2], backref = "tut_course2")
...
class TutorCount (db.Model):
    ...
    course_id = db.Column(db.String, db.ForeignKey('course.course_id'), primary_key=True)
    course = db.relationship("Course", backref = "course_tutor_id")
    ...
    def __repr__(self):
        return '<Number of students for tutor %r in course %r is %r. >' % (self.tutor_id, self.course_id,self.count_students)
...
class CourseTutorMatch(db.Model):
    ...
    tutor = db.relationship("Tutor", foreign_keys=[tutor_id_2], backref = "tutor2")
...
class TutorStudentMatch(db.Model):
    ...
    tutor_id = db.Column(db.String, db.ForeignKey('tutor.tutor_id'), primary_key=True)
    tutor = db.relationship("Tutor", backref = "tutor1")
...
class MondayRoomAvailability(db.Model):
    ...
    room_id = db.Column(db.String, db.ForeignKey('room.room_id'), primary_key=True)
...
class WednesdaRoomAvailability(db.Model):
